...
def d_brush(event):
    global lastx, lasty
    draw.create_line((lastx, lasty, event.x, event.y), fill=color, width = 5)
    lastx, lasty = event.x, event.y
...
erase = Button(color_label, bg="#A4CCCB", text="Clear all", command=lambda: clear("all"))
...
erase.grid(row=7, column=0, columnspan=3, sticky=(N, W, S, E))
